...
class Node:
    ...
    def __init__(self, id, node_type, depth, gate_type):
        ...
        if self.node_type == 2:  
            ...
            self.features['failure_probability'] = 0.0
    ...
    def generate_children(self, node):
        ...
        for _ in range(num_children):
            if node.depth + 1 < self.max_depth:
                gate_type = random.choice([1, 2])  # Ensure intermediate events have gate type 1 or 2
                node_type = 1
            else:
                gate_type = 0  # Basic events must have gate type 0
                node_type = 2
...
class Generator:
    ...
    def generate_single_model(self):
        ...
        return Converter.convert_to_graph(FT)
    ...
    def generate_data(self):
        ...
        if not nx.check_planarity(self.graph)[0]:  # Added planarity check
            raise ValueError("Generated graph is not planar")  # Raise error if not planar
        
        # Calculate in-degree, out-degree, and node centrality
        in_degrees = dict(self.graph.in_degree())
        out_degrees = dict(self.graph.out_degree())
        centrality = nx.degree_centrality(self.graph)

        for node in self.graph.nodes():
            self.graph.nodes[node]['in_degree'] = in_degrees[node]
            self.graph.nodes[node]['out_degree'] = out_degrees[node]
            self.graph.nodes[node]['centrality'] = centrality[node]

        # Print node features in the console
        for node, data in self.graph.nodes(data=True):
            print(f"Node ID: {node}, Features: {data}")
