...
gcn_model = GCN(train_data.num_features, 128, 64).to(device)
...
for epoch in range(num_epochs):
    ...
    # GCN forward pass
    node_embeddings = gcn_model(train_data.x.to(device), train_data.edge_index.to(device))
...
with torch.no_grad():
    ...
    auc = roc_auc_score(edge_labels, edge_probs)
